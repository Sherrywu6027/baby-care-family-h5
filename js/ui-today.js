...
var UIToday = (function () {
  ...
  function renderRecentTimeline(babyId) {
    DB.getEventsByDay(babyId, new Date().toISOString().slice(0, 10)).then(function (events) {
      ...
      events.slice(0, 5).forEach(function (e) {
        ...
        if (e.note) html += '<div class="ti-detail">' + e.note + '</div>';
  ...
  function buildRecorderText(event) {
    var name = event && event.recorded_by_name ? event.recorded_by_name : '未命名成员';
    var time = event && event.created_at ? new Date(event.created_at).toLocaleString('zh-CN') : '未知时间';
    return '记录人：' + name + ' · 记录时间：' + time;
  }
  ...
  function startSummary(babyId) {
    Calc.calcToday(babyId).then(function (s) {
      ...
      html += summaryCard((s.diaperCount || 0) + '次', '尿片' + (s.diaperStool > 0 ? ' 💩' + s.diaperStool : ''));
      el.innerHTML = html;
    });
  }
    ...
    function renderTimers(states) {
      ...
      if (states.length === 0) {
        if (area.dataset.mode !== 'countdown') {
          area.innerHTML = '<div class="countdown-card placeholder" id="countdown-card"><div class="label">距上次喝奶</div><div class="time" id="countdown-time">--</div><div class="sub" id="countdown-sub"></div></div>';
  ...
  function buildActiveTimersHtml(main, others) {
    ...
    if (main.type === 'milk_direct') {
      ...
      html += '<div class="direct-sides"><div class="direct-side"><div class="side-name">左胸</div><div class="side-val" id="active-main-left"></div></div>';
      html += '<div class="direct-side"><div class="side-name">右胸</div><div class="side-val" id="active-main-right"></div></div></div>';
  ...
  function updateActiveTimersContent(main, others) {
    ...
    setNodeText(mainTime, Timer.formatElapsed(main.totalMs || main.elapsed));
  ...
  function buildDirectTimerPageHtml(baby, active) {
    ...
    html += '<h2>' + ((baby && baby.name ? baby.name : '宝宝') + ' 的亲喂跑表') + '</h2>';
    ...
    if (!active) {
      ...
      html += '<div class="direct-page-total" id="direct-total-elapsed">' + Timer.formatElapsed(active.totalMs || active.elapsed) + '</div>';
  ...
  function bindDirectTimerLiveState(babyId) {
    ...
    directTimerUnsub = Timer.onTick(function (states) {
      ...
      if (totalEl) totalEl.textContent = Timer.formatElapsed(state.totalMs || state.elapsed);
  ...
  function refreshGrowthReferenceWithBaby(baby) {
    ...
    if (!baby || !baby.birthday) {
      if (hasHeight || hasWeight) {
        ...
        el.textContent = '设置出生日期后可显示中国婴幼儿参考身高和体重；本记录只保存日期';
      }
      return;
    }
  ...
  function openTimerDetail(type) {
    ...
    else App.toast('先做亲喂主流程，这块下一轮再细抠');
  }
