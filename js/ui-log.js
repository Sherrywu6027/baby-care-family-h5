var UILog = (function () {
  ...
  function loadEvents(babyId) {
    DB.getAllEvents(babyId).then(function (events) {
      ...
      var filtered = currentFilter === 'all'
        ? events
        : events.filter(function (event) { return event.type === currentFilter; });
      ...
      Object.keys(groups).sort(function (a, b) {
        ...
        groups[dateKey].forEach(function (event) {
          ...
          html += '<div class="timeline-item" onclick="UILog.editEvent(\'' + event.id + '\')" style="cursor:pointer">';
          html += '<div class="ti-time">' + formatTimelineTime(event) + '</div>';
          html += '<div class="ti-icon">' + type.icon + '</div>';
  ...
  function buildRecorderText(event) {
    ...
    return '添加人：' + escapeHtml(name) + ' · 添加时间：' + escapeHtml(time);
  }

  function formatTimelineTime(event) {
    if (!event) return '--:--';
    if (event.type === 'weight') return '日期';
    return event.start_time ? Calc.formatTime(event.start_time) : '--:--';
  }
  ...
  function editEvent(id) {
    DB.getEvent(id).then(function (event) {
      ...
      var parts = TimeUtil.toChinaDateParts(event.start_time);
      var timeStr = parts.timeKey.slice(0, 5);
      var html = '<div class="modal-overlay" onclick="if(event.target===this)UIToday.closeModal()"><div class="modal-sheet">';
      html += '<div class="modal-handle"></div><div class="modal-title">' + type.icon + ' ' + escapeHtml(type.label) + ' / 编辑</div>';
      ...
      if (event.type === 'formula' || event.type === 'milk_bottle' || event.type === 'pump') {
        html += '<div class="form-group"><label class="form-label">容量（ml）</label><input type="number" class="form-input" id="edit-amount" value="' + escapeAttr(event.amount_ml || '') + '" inputmode="numeric"></div>';
      }
      ...
      if (event.type === 'sleep' || event.type === 'pump') {
        html += '<div class="form-group"><label class="form-label">时长（分钟）</label><input type="number" class="form-input" id="edit-duration" value="' + escapeAttr(event.duration_min || '') + '" inputmode="numeric"></div>';
      }

      if (event.type === 'milk_direct') {
        html += '<div class="form-row"><div class="form-group"><label class="form-label">左侧（分钟）</label><input type="number" class="form-input" id="edit-left" value="' + escapeAttr(event.left_min || 0) + '" inputmode="numeric" oninput="UILog.refreshEditDirectTotal()"></div><div class="form-group"><label class="form-label">右侧（分钟）</label><input type="number" class="form-input" id="edit-right" value="' + escapeAttr(event.right_min || 0) + '" inputmode="numeric" oninput="UILog.refreshEditDirectTotal()"></div></div>';
      ...
      if (event.type === 'weight') {
        html += '<div class="form-group"><label class="form-label">身高（cm，可选）</label><input type="number" step="0.1" class="form-input" id="edit-height" value="' + escapeAttr(event.height_cm || '') + '"></div>';
        html += '<div class="form-group"><label class="form-label">体重（kg，可选）</label><input type="number" step="0.01" class="form-input" id="edit-weight" value="' + escapeAttr(event.weight_kg || '') + '"></div>';
      }
      ...
      if (event.type === 'weight') {
        html += '<div class="form-group"><label class="form-label">补充备注</label><input type="text" class="form-input" id="edit-extra-note" value="' + escapeAttr(event.extra_note || '') + '"></div>';
      }
      html += '<button class="btn-primary" onclick="UILog.saveEdit(\'' + id + '\')">保存修改</button>';
      html += '<button class="btn-danger" style="margin-top:8px" onclick="UILog.deleteEvent(\'' + id + '\')">删除这条记录</button>';
      html += '</div></div>';
