var AppNotifications = (function () {
  ...
  function refresh(options) {
    ...
    refreshing = Promise.all([
      ...
      var readIds = normalizeIdList(results[3]);
      var knownIds = normalizeIdList(results[4]);
      var systemEnabled = results[5] === true;
      ...
      if (!authState.loggedIn || !Sync.isConfigured || !Sync.isConfigured()) {
        return persistState([], readIds, [], systemEnabled, options);
      }
      ...
      return Promise.all([
        ...
        var items = buildNotificationItems(pendingRequests, joinState, pendingJoinCode);
        return persistState(items, readIds, knownIds, systemEnabled, options);
      });
    }).catch(function () {
      return state;
    }).finally(function () {
  ...
  function persistState(items, readIds, knownIds, systemEnabled, options) {
    options = options || {};
  ...
  function buildNotificationItems(pendingRequests, joinState, pendingJoinCode) {
    var items = [];
